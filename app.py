...
def show_manage_inventory():
    ...
    if manage_option == "Add Medicine":
        ...
        if selected_medicine:
            ...
            expiry_year = st.selectbox("Update Expiry Year", range(datetime.now().year, datetime.now().year + 10), index=datetime.strptime(medicine[4], '%Y-%m-%d').year - datetime.now().year)
            expiry_month = st.selectbox("Update Expiry Month", range(1, 13), index=datetime.strptime(medicine[4], '%Y-%m-%d').month - 1)
...
def show_manage_sales():
    ...
    if st.button("Record Sale"):
        medicine_id = next(item[0] for item in inventory if item[1] == medicine)
        if int(quantity) > inventory[medicine_id - 1][3]:
            st.error("Insufficient stock to complete the sale.")
        else:
            manage_sales((medicine_id, medicine), quantity)
